...
class Table extends Component {
    ...
    addRandom = () => {
        this.setState((previousState) => {
            let remainingContacts = copyOfJson.filter(contact => !previousState.contactsToRender.includes(contact));
            let randomNumber = Math.floor(Math.random() * remainingContacts.length);
            
            if(remainingContacts.length > 0) {
                return {
                    contactsToRender: [remainingContacts[randomNumber], ...previousState.contactsToRender]
                }
            }
        })
    }
    ...
    sortByName = () => {
        this.setState((previousState) => {
            let sortedByName = previousState.contactsToRender.sort((a, b) => a.name.localeCompare(b.name));
            return {
    ...
    sortByPopularity = () => {
        this.setState((previousState) => {
            let sortedbyPopularity = previousState.contactsToRender.sort((a, b) => b.popularity - a.popularity);
            return {
    ...
    remove = (id) => {
        this.setState((previousState) => { 
            let listWithOutRemoved = previousState.contactsToRender.filter((contact) => contact.id !== id );
            return {
            ...
            <table>
                ...
                <tbody>
                {
                    ...
                    <tr key={ contact.id }>
                        ...
                        <td><button onClick={ () => this.remove(contact.id) }>Delete</button></td>
                    </tr>
                    ))
                }
                </tbody>
