...
export default function App() {
  ...
  function sortByPopularityHandler() {
    const sortedDataByPopularity = [...contacts].sort((a, b) => (b.popularity > a.popularity) ? 1 : ((a.popularity > b.popularity)? -1 : 0))
    setContacts(sortedDataByPopularity)
  }
  function sortByNameHandler() {
    const sortedDataByName = [...contacts].sort((a, b) => a.name.localeCompare(b.name))
          ...
          return (
            <tbody>
              <tr>
                ...
                <td><button onClick={() => deleteContacts(contact.id)}>Delete</button></td>
              </tr>
            </tbody>
          );
        })}
