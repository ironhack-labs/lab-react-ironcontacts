...
// })
  ...
  return (
    <div className="container">
      <h1>IronContacts</h1>
      <button className="btn btn-info" onClick={() => addRandom()}>Add Random Contact</button>
      <button className="btn btn-info" onClick={() => sortName()}>Sort by Name</button>
      <button className="btn btn-info" onClick={() => sortPopularity()}>Sort by Popularity</button>
      <table className="table caption-top">
     ...
     
        {
          allContacts.map((e) => {
            return(
              <tr key={e.id}>
                <td><img src={e.pictureUrl} alt="" style={{width:`100px`}}/></td>
                <td>{e.name}</td>
                <td>{e.popularity}</td>
              </tr>
            )
